...
benchmark_and_validate = function(streams, pred_path, network,
  ...
  ndigits = ceiling(log(nreps_S3N, base = 10))
  rep = str_pad(rep, ndigits, side = "left", pad = "0")
                                 
  save(streams, preds, obs, file = paste0(bench_res_dir, "network", network, "_rep", rep, "_initial_data.rda"))
  ...
  S3N_preproc_and_estimation(network, rep, nreps = nreps_S3N, out_dir = bench_res_dir)
  if(!onlyS3N){
    SSN_preproc_and_estimation(network, rep, nreps = nreps_SSN, out_dir = bench_res_dir, 
